...
public class KeyboardCreator : KeyboardComponent {
    ...
    // to be called for initializing the keys
    public void Init(float r, float width, Material normal, Material selected, Material pressed)
    {
        ...
        keyNormalMaterial = normal;
        keySelectedMaterial = selected;
        KeyPressedMaterial = pressed;
        ...
        //finally, set the scale (to make sure this doesn't interfere with construction):
        Vector3 globalScale = Vector3.one * width / 10f; // because the keyboard is ten units wide (ten keys)
        transform.localScale = new Vector3(globalScale.x / transform.lossyScale.x, globalScale.y / transform.lossyScale.y, globalScale.z / transform.lossyScale.z);

        //GetComponent<KeyboardRaycaster>().SetRaycastingTransform(raycaster);
    }
