...
class LeapRaycaster : GraphicRaycaster
{
    [SerializeField]
    ...
    {
        Raycast((LeapPointerEventData)eventData, resultAppendList);
    }

    protected void Raycast(LeapPointerEventData eventData, List<RaycastResult> resultAppendList)
    {
        ...
        Ray ray3D = eventData.GetRayForHand();
        ...
        { // use custom raycasting only if Curved effect is enabled
            ...
            if (eventData.pressPosition == eventData.position)
                eventData.pressPosition = remappedPosition;
        ...
        base.Raycast(eventData, resultAppendList);//(overrideEventData ? eventData : newEventData, resultAppendList);
    }
    ...
    public virtual bool RaycastToCyllinderCanvas(Ray ray3D, out Vector2 o_canvasPos, bool OutputInCanvasSpace = false)
    {
        ...
        if (Physics.Raycast(ray3D, out hit, float.PositiveInfinity, myLayerMask))
        {
            ...
            if (hit.collider.gameObject != this.gameObject && (colliderContainer == null || hit.collider.transform.parent != colliderContainer.transform))
            {
    ...
    Mesh CreateSphereColliderMesh()
    {
        ...
        if (mySettings.Angle != 0)
        {
            ...
            {//if we're not going for constant aspect, set the width of the sphere to equal width of the original canvas
    ...
    // TODO: where are these public functions called in CurvedUIRaycaster?
    //  I will need to set up similar control flow for my corresponding scripts
    #region PUBLIC
